...
private:
   ...
   // static objects
    ...
    std::vector < std::shared_ptr<GameObject2D>> m_Menu_GameObjects2D;
    std::vector < std::shared_ptr<GameObject2D>> m_Win_GameObjects2D;
    std::vector < std::shared_ptr<GameObject2D>> m_Lose_GameObjects2D;
    //map from map.txt
    std::shared_ptr<GameMap> map = NULL;

    //positions from map.txt used for characters
    Vector3 playerSpawn;
  ...
  //dialoges they start hidden
    ...
    bool dialougeIsTrigerred = false;
 
    //remove enemyies after destorying them for preformance
